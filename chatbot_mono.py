...
def run_consumer():
    ...
    for message in consumer:
        print(f"Received kafka message: {message.value}", file=sys.stderr)
        time.sleep(0.1)
...
if prompt := st.chat_input("Ask a question!"):
    ...
    produce_message("user", prompt) #user에게 인풋받아 카프카로 프로듀싱
    ...
    produce_message("assistant", full_response) # 어시스턴트가 준 답변을 프로듀싱
